# ML/DL projects
Datasets: A folder which contains the datasets used for the projects.<br> <br>
```
Datasets¬
        |----avocado-prices(A dataset that contains prices of avocados from different types and regions in different years from 2015 to 2018. The analysis and machine learning prediction on the data is [here](/avocado-prices.ipynb) )¬
        |                   |----.ipynb_checkpoints
        |                   |----avocado.csv
        |----data_driven datasets¬
        |                         |----0bf8bc6e-30d0-4c50-956a-603fc693d966.csv
        |                         |----702ddfc5-68cd-4d1d-a0de-f5f566f76d91.csv
        |                         |----4910797b-ee55-40a7-8668-10efd5c1b960.csv
        |                         |----SubmissionFormat.csv
        |----diamonds¬
        |             |----catboost_info
        |             |----diamonds_preds.ipynb
        |             |----diamonds.csv
        |----fifa19¬
        |           |----data.csv
        |           |----FIFA.ipynb
        |----netflix-shows¬
        |                  |----netflix_titles_nov_2019.csv
        |----ufcdata¬
        |            |----data.csv
        |            |----preprocessed_data.csv
        |            |----raw_fighter_details.csv
        |            |----raw_total_fight_data.csv
        |----us-minimum-wage¬
        |                    |----Minimum Wage Data.csv
        |----wine-reviews¬
        |                 |----winemag-data_first150k.csv
        |                 |----winemag-data-130k-v2.csv
        |                 |----winemag-data-130k-v2.json
        |----fake_dataset.csv
        |----weatherAUS.csv
```

